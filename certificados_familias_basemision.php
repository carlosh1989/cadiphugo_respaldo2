...
$jefeSI = Familia::where('base_misiones',$basemisiones)->where('bodega',$bodega_id)->where('huella_certificada',$tipo)->get();
...
foreach ($jefeSI as $key => $jefe) 
{
	...
	$cabezafamilia = Jefe::where('cedula',$jefe->cod_cabeza_familia)->first();
	$array = Array (
	        0 => Array (
			    $jefe->cedula,
			    $jefe->nombre_y_apellido,
			    $jefe->sector,
			    $cabezafamilia->calle_avenida,
			    $cabezafamilia->casa_edif_apto,
			    $jefe->clap,
...
if($tipo == 1)
{
	header("Content-Disposition: attachment; filename=\"certificados_carga_familiar_base_mision_".$datosBASEMISIONES->nombre_base."_".$bodega->responsable.".xls\"");
}
elseif($tipo == 0) 
{
	header("Content-Disposition: attachment; filename=\"no_certificados_carga_familiar_base_mision_".$datosBASEMISIONES->nombre_base."_".$bodega->responsable.".xls\"");
}
